...
enum SampleData {
    ...
    static func insert(into context: ModelContext) -> Int {
        // Só os alimentos de que as receitas precisam (quem apagou alimentos não os vê voltar todos).
        let foodsByName = FoodLibrary.insertMissingDefaults(in: context, only: Set(samples.flatMap { $0.items.map(\.food) }))
